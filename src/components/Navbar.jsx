    ...
    <header className={`${s.header} ${scrolled ? s.scrolled : ''}`}>
      <div className={s.inner}>
        ...
        <nav className={s.nav}>
          ...
          <NavLink to="/author" className={({isActive}) => `${s.navLink} ${isActive ? s.active : ''}`}>Автор</NavLink>
        </nav>

        {/* Centered search à la shikimori */}
        <div className={s.searchBox} ref={boxRef}>
          ...
          {open && q.trim().length >= 2 && (
            <div className={s.searchDropdown}>
              {loading && <div className={s.searchHint}>Поиск…</div>}
              {!loading && results.length === 0 && <div className={s.searchHint}>Ничего не найдено</div>}
        ...
        <div className={s.themeBox} ref={themeRef}>
          <button className={s.themeBtn} onClick={() => setThemeMenu(v => !v)} aria-label="Тема">
            {ThemeIcon}
          </button>
          {themeMenu && (
      ...
      {mobileOpen && (
        <div className={s.mobile}>
          ...
          <NavLink to="/author" className={s.mobileLink}>Автор</NavLink>
          <NavLink to="/support" className={s.mobileLink}>Поддержать ❤</NavLink>
        </div>
      )}
    </header>
  );
